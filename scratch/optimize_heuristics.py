...
def evaluate_params(cache, filter_size, interaction_radius, walk_away_dist, drop_window, return_margin, after_margin, end_buffer):
    correct = 0
    total = len(cache)
    
    # Pre-cluster static slots and compute occupancy counts for each video to avoid redundant work
    # We will do a simple run over all videos
    for key, video_data in cache.items():
        ...
        # Smooth counts with median filter
        ...
        for track_id, pickups in potential_pickups.items():
            ...
            for pickup in pickups:
                ...
                if avg_count_after > baseline_capacity - after_margin:
                    reasons.append("avg_count_after > baseline - after_margin")
...
def main():
    ...
    print(f"Loaded cache. Size={len(cache)}. Starting grid search...")
    
    # Try a few combinations
    best_correct = 0
    best_params = None
    
    # Grid search subsets to run extremely fast on a few videos first or run on all since it takes 1 sec
    # Let's search over standard reasonable values
                        ...
                        for after_margin in [0.3, 0.4, 0.5]:
                            for end_buffer in [15, 30]:
                                correct = evaluate_params(
                                    cache, filter_size, interaction_radius, walk_away_dist,
                                    drop_window, return_margin, after_margin, end_buffer
                                )
                                print(f"Params: filter={filter_size}, radius={interaction_radius}, walk={walk_away_dist}, drop={drop_window}, ret={return_margin}, after={after_margin}, end={end_buffer} => {correct}/{len(cache)} ({100*correct/len(cache):.2f}%)")
                                if correct > best_correct:
                                    best_correct = correct
                                    best_params = (filter_size, interaction_radius, walk_away_dist, drop_window, return_margin, after_margin, end_buffer)
                                    print(f"*** NEW BEST: {best_correct}/{len(cache)} ({(100*best_correct/len(cache)):.2f}%)")
